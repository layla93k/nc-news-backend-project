...
exports.getAllArticles = (req, res, next) => {
  if (req.query.p === undefined && req.query.limit === undefined) {
    ...
    if (
      /\d/.test(req.query.p) === false ||
      /\d/.test(req.query.limit) === false
    ) {
      ...
      fetchAllArticles()
        .then((articles) => {
          const paginatedArticles = paginateData(
            articles,
            pageNum,
            responseLimit
          );
          const dataLength = paginatedArticles.data.length;
          if (pageNum > Math.ceil(dataLength / responseLimit) + 1) {
...
exports.patchVotes = (req, res, next) => {
  ...
  editVotes(article_id, incVoteNum)
    .then((editedArticle) => {
